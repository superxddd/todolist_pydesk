...
class DetailPanel(ttk.Frame):
    def __init__(self, master, **kwargs):
        ...
        ttk.Label(self, text="任务详情", style="Section.TLabel").pack(anchor="w", padx=8, pady=(8, 4))
        ...
        ttk.Label(self, text="子任务", style="Section.TLabel").pack(anchor="w", padx=8, pady=(12, 4))
        self.subtasks = ttk.Label(self, text="", justify="left")
        self.subtasks.pack(anchor="w", fill="x", padx=8)
        ttk.Label(self, text="AI 备注", style="Section.TLabel").pack(anchor="w", padx=8, pady=(12, 4))
    ...
    def show_task(self, task: Task | None) -> None:
        ...
        self.meta_label.configure(text=f"状态：{task.status}  优先级：{task.priority}  截止：{due}  标签：{tags}")
        self.description.configure(text=task.description or "无描述")
        lines = [f"{'✓' if item.done else '•'} {item.title}" for item in task.subtasks] or ["暂无子任务"]
        self.subtasks.configure(text="\n".join(lines))
